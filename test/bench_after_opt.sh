...
run_bench() {
    ...
    for i in 1 2 3; do
        ...
        ab -n $REQUESTS -c $CONCURRENCY -k -e /tmp/ab_stats_opt.csv -H "$UA" $extra_args "$TARGET" > /tmp/ab_out_opt.txt 2>&1 &
        local ab_pid=$!
        sleep 0.5
        ...
        local rps=$(grep "Requests per second" /tmp/ab_out_opt.txt | awk '{print $4}')
        local tpr=$(grep "Time per request.*mean" /tmp/ab_out_opt.txt | head -1 | awk '{print $4}')
        local p99=$(awk -F, 'NR==102{printf "%.0f", $2}' /tmp/ab_stats_opt.csv 2>/dev/null)
        local fail=$(grep "Failed requests" /tmp/ab_out_opt.txt | awk '{print $3}')
        [ -z "$fail" ] && fail=0
    ...
    echo "  >>> BEST: RPS=${best_rps} TPR=${best_tpr}ms P99=${best_p99}ms RSS=${best_mem}MB" | tee -a $RESULT_FILE
    echo "" | tee -a $RESULT_FILE
}

cp $WAF_CONFIG ${WAF_CONFIG}.bak_opt
...
cp ${WAF_CONFIG}.bak_opt $WAF_CONFIG
...
cp ${WAF_CONFIG}.bak_opt $WAF_CONFIG
set_config "cc_check" "off"
mv $RULE_DIR/cdnip.rule $RULE_DIR/cdnip.rule.bak_opt 2>/dev/null
restart_nginx
run_bench "D-proxy-on-nocdnip" ""
mv $RULE_DIR/cdnip.rule.bak_opt $RULE_DIR/cdnip.rule 2>/dev/null

# 恢复
cp ${WAF_CONFIG}.bak_opt $WAF_CONFIG
rm -f ${WAF_CONFIG}.bak_opt
restart_nginx
