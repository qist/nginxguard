...
|------|-------|-----|-----|-----|---------|
| NginxGuard 全关（基线） | 34,874 | 194% | 1,174 MB | 18ms | — |
| NginxGuard 全开（无 CC/POST） | 20,485 | 286% | 1,183 MB | 31ms | 41.3% |
| NginxGuard + CC（POST 关闭） | 24,056 | 298% | 1,180 MB | 33ms | — |
| NginxGuard + POST（CC 关闭） | 10,445 | 313% | 1,189 MB | 87ms | — |
| NginxGuard 全开（生产） | 24,005 | 310% | 1,180 MB | 60ms | 31.2% |

> **结论**：NginxGuard 全开相比全关的吞吐下降约 31%，主要开销来自每请求的规则匹配。当前规则文件较上一版本增长 2-3 倍（args 95→227 条，url 92→292 条），合并正则从约 2KB 增至 5.4KB，正则引擎匹配开销相应增加。解码优化已通过 `has_encode_markers` 快速特征检测，使正常流量（无编码字符）跳过递归解码和 JS unicode 解码，零额外开销。CC 防护生效后 IP 自动封禁 600 秒，后续请求直接 403 快速返回。POST 检测因需读取请求体开销最大。内存增加约 10MB（规则缓存 + 共享字典）。
