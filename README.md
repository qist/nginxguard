...
└── rule-config/
    ...
    └── domains/                # 域名专属规则目录
        └── www.example.com/
            ...
            ├── whiteurl.rule      # URL白名单（纯路径默认跳过URL检测，可选配置跳过指定检测项）
...
| `white_url_check` | 白名单 URL 检测（纯路径默认跳过 URL 路径检测；可选配置跳过指定检测项） | `config_white_url_check` |
...
| `whiteurl.rule` | `white_url_check()` | URL 白名单（纯路径默认跳过 URL 路径检测；可选配置跳过指定检测项） |
...
### 白名单 URL（whiteurl.rule）

`whiteurl.rule` 支持**两种格式**，可按 URL 路径配置跳过哪些检测项。

#### 格式 1：纯路径（默认跳过 URL 路径检测）
...
纯路径格式**只跳过 `url_attack`（URL 路径检测）**，其他所有检测照常执行：
...
**场景 2：旧版 API 放行 UA 和 URL 检测，但保留 POST/Cookie 检测**
```bash
# rule-config/whiteurl.rule
/legacy/ user_agent,referer,url_attack,url_args
```
...
5. black_ip_check     → 静态黑名单 IP
       ↓
6. white_url_check    → 白名单 URL（返回跳过的检测项配置）
       ↓
